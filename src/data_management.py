...
def create_data_adapter(config: dict[str, Any]) -> BaseDataAdapter:
    ...
    if data_format.upper() in ["UCLA-LES", "UCLA_LES", "DALES", "RICO"]:
        ...
        raise ValueError(f"Unknown data format: {data_format}. Supported formats: 'UCLA-LES', 'MONC'")


def load_cloud_field_from_adapter(adapter: BaseDataAdapter, timestep: int, config: dict[str, Any]) -> CloudField:
    """
...
def calculate_mean_velocities_from_adapter(adapter: BaseDataAdapter, timestep: int) -> tuple:
    """
...
def load_cloud_field_from_file(file_path, file_names, timestep, config):
    ...
    cloud_field = CloudField(
        l_data, u_data, v_data, w_data, p_data, theta_l_data, q_t_data, r_data, timestep, config, xt, yt, zt
    )
