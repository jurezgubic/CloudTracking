...
def initialize_netcdf(file_path, zt, ring_count, env_aloft_levels=40, config=None):
    ...
    with Dataset(file_path, "w", format="NETCDF4") as root_grp:
        ...
        root_grp.createVariable("track_id", "i8", ("track",))
        ...
        root_grp.createVariable("merged_into", "i4", ("track", "time"), fill_value=-1)

        root_grp.createVariable("merges_count", "i4", ("track", "time"), fill_value=0)
        root_grp.createVariable("splits_count", "i4", ("track", "time"), fill_value=0)
        root_grp.createVariable("split_from", "i4", ("track", "time"), fill_value=-1)
...
def write_cloud_tracks_to_netcdf(
    tracks,
    track_id_to_index,
    tainted_tracks,
    ...
    with Dataset(file_path, "a") as root_grp:
        ...
        for track_id, track in tracks.items():
            ...
            if clouds_at_timestep:
                ...
                base_radius_prescribed_var[i, timestep] = (
                    np.sqrt(cloud.cloud_base_area / np.pi) if cloud.cloud_base_area > 0 else np.nan
                )
                ...
                if timestep > 0 and track:
                    last_cloud = track[-1]
                    if (
                        (last_cloud.timestep == timestep - 1)
                        and (not last_cloud.is_active)
                        and (last_cloud.merged_into is not None)
                    ):
