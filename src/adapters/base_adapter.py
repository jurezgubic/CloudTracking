    ...
    def validate_data(self, data: dict[str, np.ndarray]) -> None:
        ...
        for key in ["u", "v", "w", "p", "theta_l", "q_t", "r"]:
            if data[key].shape != shape_3d:
                raise ValueError(f"Shape mismatch: {key} has shape {data[key].shape}, expected {shape_3d}")
        ...
        if np.any(data["theta_l"] < 100) or np.any(data["theta_l"] > 1000):
            raise ValueError(
                f"Theta_l values out of reasonable range: [{data['theta_l'].min():.1f}, {data['theta_l'].max():.1f}] K"
            )
        if np.any(data["p"] < 1000) or np.any(data["p"] > 150000):
            raise ValueError(
                f"Pressure values out of reasonable range: [{data['p'].min():.1f}, {data['p'].max():.1f}] Pa"
            )
