...
class MONCAdapter(BaseDataAdapter):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        print(
            f"  Reference theta profile: {len(self.z_thref)} points, "
    ...
    def _discover_files(self) -> None:
        ...
        if not files:
            raise FileNotFoundError(f"No MONC output files found matching pattern: {self.data_path / glob_pattern}")
    ...
    def _compute_reference_profiles(self, zn: np.ndarray) -> None:
        ...
        print("  Reference profiles computed:")
    ...
    def load_timestep(self, timestep: int) -> dict[str, np.ndarray]:
        ...
        try:
            ...
            warnings.warn(f"Data validation warning: {e}", stacklevel=2)

        gc.collect()

        return data

    def _interpolate_w_to_scalar_levels(self, w_on_z: np.ndarray, z_w: np.ndarray, zn: np.ndarray) -> np.ndarray:
        """
