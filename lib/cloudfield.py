        ...
        for batch_start in range(0, len(regions), batch_size):
            ...
            for region_idx, region in enumerate(batch_regions):
                if region.area >= config['min_size']:
                    ...
                    points_np = np.column_stack([ # Renamed to points_np for clarity
                        xt[point_indices[:, 2]],
                        yt[point_indices[:, 1]],
                        zt[point_indices[:, 0]]
                    ])
                    # points = [tuple(p) for p in points]  # OLD: Convert to list of tuples # ToDo (optimisation)
                    ...
                    clouds[cloud_id] = Cloud(
                        ...
                        points=points_np, # MODIFIED: Pass NumPy array directly
