...
class CloudField:
    ...
    def __init__(
        self, l_data, u_data, v_data, w_data, p_data, theta_l_data, q_t_data, r_data, timestep, config, xt, yt, zt
    ):
        ...
        if config["b_switch"]:
            labeled_array = self.identify_regions(
                l_data, w_data, config, theta_l_data=theta_l_data, q_t_data=q_t_data, p_data=p_data, r_data=r_data
            )
        else:
    ...
    def _calculate_environment_mass_flux(
        self, labeled_array, p_data, theta_l_data, l_data, q_t_data, r_data, w_data, config
    ):
    ...
    # @profile
    def create_clouds_from_labeled_array(
        self,
        updated_labeled_array,
        l_data,
        u_data,
        ...
        {z_val: idx for idx, z_val in enumerate(zt)}
        ...
        if env_aloft_sampling_mode == "fixed_mean_area" and fixed_sampling_offsets is None:
            ...
            if valid_regions:
                ...
                print("--- Fixed Mean Area Sampling Initialized ---")
            ...
            for region_idx, region in enumerate(batch_regions):
                if region.area >= config["min_size"]:
                    ...
                    points = np.column_stack(
                        [xt[point_indices[:, 2]], yt[point_indices[:, 1]], zt[point_indices[:, 0]]]
                    )
                    ...
                    for z_level_idx, _count in zip(unique_z_levels, z_level_counts):
                    ...
                    if env_aloft_mode == "flat" and len(valid_z_locals) > 0:
                    ...
                    if env_aloft_sampling_mode == "fixed_mean_area" and fixed_sampling_offsets is not None:
                        if env_aloft_mode != "flat":
                            raise ValueError(
                                "Error: 'fixed_mean_area' sampling requires 'env_aloft_mode' to be 'flat'."
                            )
                        ...
                        max_z_idx = np.max(abs_z_tops) if len(abs_z_tops) > 0 else int(max_height / (zt[1] - zt[0]))
                    ...
                    for z_level_idx in unique_z_levels:
                        ...
                        if periodic_rings:
                            ...
                            np.pad(env_slice, padw, mode="wrap")
                        else:
                            cloud_pad = np.pad(cloud_slice, padw, mode="constant", constant_values=False)
                            np.pad(env_slice, padw, mode="constant", constant_values=False)
    ...
    def _compute_nip(self, config):
        ...
        float(config.get("nip_Vref_floor", 1.0))
