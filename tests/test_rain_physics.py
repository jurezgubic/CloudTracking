...
from utils.physics import _density_with_loading, _saturation_adjustment_newton, calculate_physics_variables
...
class TestRainPhysics(unittest.TestCase):
    ...
    def test_rain_excluded_from_saturation(self):
        ...
        temps_no_rain, rhos_no_rain, _ = calculate_physics_variables(
            p_values, theta_l_values, q_l, q_v, w_values, self.horizontal_resolution_squared, r_rain_values=None
        )
        ...
        np.testing.assert_almost_equal(
            temps_zero_rain[0], temps_no_rain[0], decimal=6, err_msg="Zero rain should give same temperature as no rain"
        )
        np.testing.assert_almost_equal(
            rhos_zero_rain[0], rhos_no_rain[0], decimal=6, err_msg="Zero rain should give same density as no rain"
        )
        ...
        temps_with_rain, rhos_with_rain, _ = calculate_physics_variables(
            p_values, theta_l_values, q_l, q_v, w_values, self.horizontal_resolution_squared, r_rain_values=r_rain
        )
        ...
        self.assertLess(
            temp_diff,
            0.5,  # Less than 0.5 K difference
        ...
        self.assertGreater(rhos_with_rain[0], rhos_no_rain[0], "Density should increase with rain water loading")
    ...
    def test_rain_increases_density_loading(self):
        ...
        self.assertGreater(rho_with_rain, rho_no_rain, "Rain should increase density through water loading")
    ...
    def test_saturation_adjustment_with_reduced_condensable(self):
        ...
        self.assertGreater(
            temp_diff,
            0.5,  # At least 0.5 K difference
            f"Temperature difference should be significant: {temp_diff:.2f} K",
        )
        self.assertLess(
            temp_diff,
            15.0,  # No more than 15 K difference
    ...
    def test_vectorized_rain_handling(self):
        ...
        self.assertGreater(rho_trend, 0, "Density should generally increase with rain amount")
