...
def create_test_cloud(
    cloud_id, x, y, z, size=10, is_active=True, age=0, mean_u=None, mean_v=None, mean_w=None, zt=None
):
    """
    ...
    surface_points = np.array(
        [[x + dx, y + dy, z] for dx in range(-2, 3) for dy in range(-2, 3)][:n_surface_points], dtype=np.float32
    )
