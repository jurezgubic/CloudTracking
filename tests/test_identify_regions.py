...
class TestIdentifyRegionsLCondition(_IdentifyRegionsHelper, unittest.TestCase):
    ...
    def test_single_cloud_above_threshold(self):
        ...
        self.assertTrue(np.all(labeled[2, 3:6, 3:6] > 0),
                        "All cloudy points should be labeled")
        self.assertEqual(np.sum(labeled > 0), 9,
                         "Only the 9 cloudy points should be labeled")
...
class TestIdentifyRegionsWSwitch(_IdentifyRegionsHelper, unittest.TestCase):
    ...
    def test_w_switch_removes_downdraft_cloud(self):
        ...
        config = {'l_condition': 5e-4, 'w_switch': True,
                  'w_condition': 0.0, 'b_switch': False}
        cf = self.make_field()
        labeled = cf.identify_regions(l_data, w_data, config)

        self.assertEqual(np.max(labeled), 0,
                         "Downdraft cloud should be excluded by w_switch")
    ...
    def test_w_switch_false_keeps_downdraft(self):
        ...
        self.assertEqual(np.max(labeled), 1,
                         "Downdraft cloud should be kept when w_switch=False")
    ...
    def test_w_switch_splits_updraft_from_downdraft(self):
        ...
        self.assertEqual(np.sum(labeled > 0), 10,
                         "Only 10 updraft points should survive")
    ...
    def test_w_condition_nonzero_threshold(self):
        ...
        config = {'l_condition': 5e-4, 'w_switch': True,
                  'w_condition': 0.5, 'b_switch': False}
        cf = self.make_field()
        labeled = cf.identify_regions(l_data, w_data, config)

        self.assertEqual(np.max(labeled), 0,
                         "Cloud with w < w_condition should be excluded")
...
class TestFindBoundaryMerges(_IdentifyRegionsHelper, unittest.TestCase):
    ...
    def test_north_south_merge(self):
        ...
        self.assertIn((2, 1), merges,
                      "North label 2 and south label 1 should be merged")
    ...
    def test_east_west_merge(self):
        ...
        self.assertEqual(len(merges), 1, "Should detect 1 merge pair")
        self.assertIn((2, 1), merges,
                      "East label 2 and west label 1 should be merged")
    ...
    def test_no_merge_far_apart(self):
        ...
        self.assertEqual(len(merges), 0,
                         "Distant boundary fragments should not merge")
    ...
    def test_no_merge_without_boundary_contact(self):
        ...
        self.assertEqual(len(merges), 0,
                         "Interior cloud should not trigger merge")
...
class TestUpdateLabelsForMerges(_IdentifyRegionsHelper, unittest.TestCase):
    ...
    def test_chain_merge_partial_unification(self):
        ...
        self.assertEqual(unique_labels, {1, 2},
                         "Sequential processing leaves labels {1, 2} not fully unified")
